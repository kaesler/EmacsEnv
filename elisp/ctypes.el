;;
;;The CTypes package enhance font-locking of custom defined types in C
;;and C++ files.
;;
;;
;;The new features of V1.1 are:
;;
;;* Two new variables, designed to be added to the `Local Variables'
;;section at the end of source files, makes it possible to define new
;;types and to load type files when a file a opened.  This makes it easy
;;to have a project-specific type file that is read only when needed.
;;
;;* `typedef unsigned char byte_t' now works.
;;
;;* Lots of small enhancement both to the code and to the documentation.
;;
;;	-- Anders
;;
;;
...
the content of the buffer looking for types."
  (let ((added nil))
	...
	(setq added t))
    (let ((lst (or (and (stringp ctypes-read-files-at-load)
			(list ctypes-read-files-at-load))
		   ctypes-read-files-at-load)))
      (while lst
	(if (and (stringp (car lst))
		 (ctypes-read-file (car lst) nil t))
	    (setq added t))
	(setq lst (cdr lst))))
    (if ctypes-auto-parse-mode
	(ctypes-buffer))
...
(defun ctypes-build-regexp (lst)
  ...
  (let ((regexp "")
    ...
    (while lst
      (if first
	  (setq regexp (car lst))
	(setq regexp (concat regexp "\\|" (car lst))))
      (setq first nil)
      (setq lst (cdr lst)))
    regexp))
