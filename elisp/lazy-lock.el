;;From Simon.Marshall@esrin.esa.it Wed Mar  5 09:58:14 1997
;;x-gateway: relay3.UU.NET from gnu-emacs-sources to gnu.emacs.sources; Wed, 5 Mar 1997 06:03:48 EST
;;Date: Wed, 5 Mar 1997 10:49:38 GMT
;;Message-ID: <9703051049.AA01979@mail.esrin.esa.it>
;;From: Simon.Marshall@esrin.esa.it (Simon Marshall)
;;Subject: lazy-lock (2.07.01): Lazy fontification for fast Font Lock mode.
;;X-Mailer: Emacs 19.34 (RMail/Mail)
;;Reply-To: Simon.Marshall@esrin.esa.it (Simon Marshall)
;;Newsgroups: gnu.emacs.sources
;;Path: atria.com!cam-news-feed2.bbnplanet.com!cpk-news-hub1.bbnplanet.com!cam-news-hub1.bbnplanet.com!news.bbnplanet.com!howland.erols.net!news.mathworks.com!news.kei.com!news.thenet.net!uunet!in2.uu.net!relay3.uu.net!wendy-fate.uu.net!gnu-emacs-sources
;;Sender: gnu-emacs-sources-request@prep.ai.mit.edu
;;Lines: 1148

;;Hi folks,

;;Here is version 2.07.01 of lazy-lock.el.  This version works with Emacs 19.30
;;and later.  (Currently XEmacs does not have the features to support this
;;version of lazy-lock.el.)  See also fast-lock.el, now distributed with both
;;Emacs and XEmacs.

;;Changes since version 2.06 (i.e., that distributed with Emacs 19.34):

;;- Deferred on-the-fly fontification updates the buffer corresponding to true
;;  syntactic context.  This means if you insert a string or comment character,
;;  the fontification of all subsequent lines are regenerated to reflect their
;;  new syntactic context.  See the new variable `lazy-lock-defer-contextually'.

;;  Previously, Lazy Lock mode deferred on-the-fly fontification behaved in the
;;  same way as basic Font Lock mode on-the-fly fontification.  Occasionally
;;  someone asks that on-the-fly fontification fontify more than the modified
;;  line, say, a few lines above and/or below.  I've always balked at this as it
;;  seems no less arbitrary than the current behaviour and no more correct.
;;  I think that this is a better solution, so I'd be interested in any feedback
;;  concerning how this feature performs.

;;- Stealth fontification can be suspended when the system load becomes high.
;;  This prevents Emacs from slowing down other things that are running on the
;;  same system.  See the new variable `lazy-lock-stealth-load'.

;;  Of course, "system load" is not supported on all systems, and, where it is
;;  supported, differs from system to system.  As above, I'd be interested in
;;  any feedback concerning how this feature performs.
...
;;  These changes were necessary to make the different new deferral options
;;  specifiable by more intuitively named variables.
...
;;See the comment header for more information and installation instructions, and
;;the comment header and command `lazy-lock-mode' for usage instructions.

;;Please report problems, etc., via M-x lazy-lock-submit-bug-report RET.

;;Enjoy, Simon.
;;_______________________________________________________________________________
;;Simon Marshall, ESRIN (ESA), Via Galileo Galilei, Frascati 00044 (Roma), Italia
;; "Under that heart of stone beat muscles of pure flint."  Sid Waddell (BSkyB)
;;   Email: Simon.Marshall@esrin.esa.it  Voice: +39 6 941 80 583  Fax: 80 280
...
;    (reporter-submit-bug-report "simon@gnu.ai.mit.edu" "lazy-lock 2.07.01"
...
(defvar lazy-lock-defer-contextually 'syntax-table
