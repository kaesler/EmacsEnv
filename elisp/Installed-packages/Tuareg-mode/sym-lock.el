...
(defun sym-lock-face (pat num pos obj &optional override)
  "Define an entry for the font-lock table which substitutes PAT (at NUM) by
OBJ under `sym-lock-adobe-symbol-face'. The face extent will NOT become
atomic."
  (list (concat "\\(" (substring pat 0 pos) "\\)"
		(substring pat pos (length pat)))
	(1+ num) (sym-lock-remap-face pat pos obj nil) override))

(defun sym-lock-atom (pat num pos obj &optional override)
  "Define an entry for the font lock table which substitutes PAT (at NUM) by
...
(defun font-lock-set-defaults (&optional explicit-defaults)
  (when
      (and
       ...
       font-lock-keywords
       (if (and sym-lock-enabled (boundp 'sym-lock-keywords)
		(listp (cadadr font-lock-keywords))
		(not (string-match
		      "sym-lock"
		      (symbol-name (face-name (cadr (cadadr
						     font-lock-keywords)))))))
	   (setq font-lock-keywords (append sym-lock-keywords
					    font-lock-keywords)) t))
    (turn-on-font-lock)))
