...
cmubackground-show and cmubackground-select."
  (interactive "s%% ")
  (let ((job-number 1)
	(job-name "%1")
	(dir default-directory))

    ;; Get a unique buffer name for this job.
    ;; Don't reuse an old background buffer.
    ;; I might want to look at it.
    ;;
    (while (get-buffer job-name)
      (setq job-name (concat "%" (setq job-number (1+ job-number)))))

    (if cmubackground-select (pop-to-buffer job-name)
      (if cmubackground-show (with-output-to-temp-buffer job-name)) ; cute
    ...
    (let ((proc (get-buffer-process
      ...
      (cond (proc (set-process-sentinel proc 'cmubackground-sentinel)
		  (message "[%d] %d" job-number (process-id proc))))
      (setq mode-name "Background")
      proc)))

(defun cmubackground-sentinel (process msg)
