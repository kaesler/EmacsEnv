...
;; Also thanks to Klaus Berndl, Rohit Namjoshi, Robert Fenk, and
;; Alex Schroeder for bug fixes and improvements.
...
(defcustom ido-merge-work-directories t
  "*Non-nil means to merge all matching directories in the work directory history.
Use \\[ido-prev-match-directory] or \\[ido-next-match-directory] to change directory."
  :type 'boolean
  :group 'ido)
...
Value is a list (ido-text dir cur-list matches).")
...
(defun ido-load-history ()
  ...
  (interactive)
  (if (and ido-save-directory-list-file (not ido-last-directory-list))
...
Removes badly formatted data and ignored directories."
  (interactive)
  (or (listp ido-last-directory-list)
      (setq ido-last-directory-list nil))
  (or (listp ido-work-directory-list)
      (setq ido-work-directory-list nil))
  (or (listp ido-work-file-list)
      (setq ido-work-file-list nil))
  (or (listp ido-dir-file-cache)
...
If INITIAL is non-nil, it specifies the initial input string."
  (let
    ...
    (while (not done)
      ...
      (if (and ido-use-merged-list (eq ido-try-merged-list t) (not ido-keep-item-list))
	  (let ((olist ido-cur-list)
		(omat ido-matches)
		(l (ido-make-merged-file-list ido-text-init (eq ido-use-merged-list 'auto))))
	    (cond
	     ((not l)
	     ...
	     (t
	      (setq ido-pre-merge-state
		    (list ido-text-init ido-current-directory olist omat))
	...
	(setq ido-cur-list (ido-make-file-list ido-default-item)))
       ((eq ido-cur-item 'dir)
	(setq ido-cur-list (ido-make-dir-list ido-default-item)))
       ((eq ido-cur-item 'buffer)
	(setq ido-cur-list (ido-make-buffer-list ido-default-item)))
       (t nil))
...
(defun ido-file-internal (method &optional fallback default prompt item initial)
  ...
  (if (not ido-enabled)
    ...
    (let (filename
	  ...
	  (ido-current-directory (expand-file-name (or default default-directory)))
	...
	(ido-record-command 'find-file-read-only filename)
	(ido-record-work-directory)
...
If no merge has yet taken place, toggle automatic merging option."
  ...
  (cond
   (ido-pre-merge-state
    (ido-set-current-directory (car (cdr ido-pre-merge-state)))
    (setq ido-text-init (or text (car ido-pre-merge-state))
	  ido-cur-list (car (cdr (cdr ido-pre-merge-state)))
	  ido-matches (car (cdr (cdr (cdr ido-pre-merge-state))))
	  ido-use-merged-list nil
   ...
   ((not ido-use-merged-list)
    (let ((ido-merge-work-directories t))
...
(defun ido-get-work-directory (&optional incr must-match how)
  (if ido-merge-work-directories
      (progn
	(setq ido-use-merged-list how ido-try-merged-list t)
	t)
...
If `ido-merge-work-directories' is set, merge directories now."
  (interactive)
  (let ((dir (ido-get-work-directory 1 ido-work-directory-match-only t)))
    (when dir
      (unless (eq dir t) (ido-set-current-directory dir))
      (setq ido-exit 'refresh)
      (setq ido-text-init ido-text)
      (setq ido-rotate-temp t)
      (exit-minibuffer))))
...
(defun ido-make-merged-file-list (text auto)
  (let (res)
...
(defun ido-file-name-all-completions (dir)
  ...
  (if (and (numberp ido-max-dir-file-cache) (> ido-max-dir-file-cache 0)
	...
	(if attr
	    (let ((ce (assoc dir ido-dir-file-cache))
		  (mtime (nth 5 attr)))
	      (if ce
		  (let ((ctime (nth 1 ce)))
		    (if (not (and (= (car ctime) (car mtime)) (= (car (cdr ctime)) (car (cdr mtime)))))
			(setq ido-dir-file-cache (delq ce ido-dir-file-cache)
			      ce nil))))
	      (if (not ce)
		  (setq ce (cons dir (cons mtime (file-name-all-completions "" dir)))
			ido-dir-file-cache (cons ce ido-dir-file-cache)))
...
(defun ido-make-file-list1 (dir &optional merged)
  ...
  (and (file-directory-p dir)
       (delq nil 
	     (mapcar
	      (lambda (name)
		(and (not (ido-ignore-item-p name ido-ignore-files t)) (if merged (cons name dir) name)))
...
\\[ido-prev-work-directory] or \\[ido-next-work-directory] cycle through the work directory history.
...
(defun ido-minibuffer-setup ()
  ...
  (when (and (boundp 'ido-completing-read) (= ido-use-mycompletion-depth (minibuffer-depth)))
