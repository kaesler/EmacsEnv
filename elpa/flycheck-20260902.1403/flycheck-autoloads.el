...
With prefix ARG, enable Global Flycheck-Annotate mode if ARG is positive; otherwise, disable it.

If called from Lisp, toggle the mode if ARG is `toggle'.
Enable the mode if ARG is nil, omitted, or is a positive number.
Disable the mode if ARG is a negative number.

Flycheck-Annotate mode is enabled in all buffers where `(lambda nil (when (flycheck-may-enable-mode) (flycheck-annotate-mode 1)))' would do it.
...
With prefix ARG, enable Global Flycheck-Lsp mode if ARG is positive; otherwise, disable it.

If called from Lisp, toggle the mode if ARG is `toggle'.
Enable the mode if ARG is nil, omitted, or is a positive number.
Disable the mode if ARG is a negative number.

Flycheck-Lsp mode is enabled in all buffers where `(lambda nil (when (flycheck-lsp--available-command major-mode) (flycheck-lsp-mode 1)))' would do it.
...
Flycheck-Eglot mode is enabled in all buffers where `(lambda nil (when (flycheck-eglot--available-p) (flycheck-eglot-mode 1)))' would do it.
